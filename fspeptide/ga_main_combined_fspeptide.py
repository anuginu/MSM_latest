...
def Get_combined_features_fspeptide():                                                     
  ...
  co=comb_features.fit_transform(trajs)
  return co
...
def main_modified(generations):
 ...
 contacts=Get_combined_features_fspeptide()
 scaler = RobustScaler()                                                           
 scaled_feature = scaler.fit_transform(contacts) 
 ...
 #starttime = time.time()
   ...
   offsprings_2_binary=[]
   count_mutation=len(population_dihedral)-len(offsprings_1)-1
